...
export default function ExpertEducationPage() {
  ...
  const trainingPrograms = [
    {
      icon: <Award className="h-8 w-8" /> import { Metadata } from 'next';
import Link from 'next/link';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
            ...
            <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
              {packages.map((pkg, index) => (
                ...
                >
                  ...
                  {pkg.comingSoon && (
                    <div className="absolute -top-4 left-0 right-0 flex justify-center">
                      <Badge className="bg-orange-600 text-white">Available Now</Badge>
                    </div>
                  )}
                  <Card className={`h-full ${pkg.recommended ? 'border-teal-600 shadow-xl' : ''} ${pkg.comingSoon ? 'opacity-75' : ''}`}>
                    ...
                    <CardContent>
                      ...
                      >
                        {pkg.comingSoon ? 'Available Now' : 'Get Started'}
                      </Button>
                    </CardContent>
                  </Card>
                </div>
...
.Value -replace "'", "'" <Laptop className="h-8 w-8" /> import { Metadata } from 'next';
import Link from 'next/link';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
...
.Value -replace "'", "'" <Briefcase className="h-8 w-8" /> import { Metadata } from 'next';
import Link from 'next/link';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
...
.Value -replace "'", "'" <Users className="h-8 w-8" /> import { Metadata } from 'next';
import Link from 'next/link';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
                  ...
                  <Card className={`h-full ${pkg.recommended ? 'border-teal-600 shadow-xl' : ''} ${pkg.comingSoon ? 'opacity-75' : ''}`}>
                    ...
                    <CardContent>
                      ...
                      {pkg.tiers && (
                        <div className="space-y-4 mb-6">
                          <div className="text-sm font-semibold text-gray-700 dark:text-gray-300">Includes:</div>
                          <ul className="space-y-2"> import { Metadata } from 'next';
import Link from 'next/link';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
...
.Value -replace "'", "'" <li key={feature} className="flex items-start gap-2">
                                <CheckCircle className="h-5 w-5 text-green-600 flex-shrink-0 mt-0.5" />
                                <span className="text-gray-700 dark:text-gray-300 text-sm">{feature}</span>
                              </li>
                            ))}
