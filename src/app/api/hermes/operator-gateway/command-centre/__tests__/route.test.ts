...
describe('GET /api/hermes/operator-gateway/command-centre', () => {
  ...
  it('returns the safe command-centre operator surface without live execution', async () => {
    ...
    expect(json.boardDecisionPanel.currentDecision).toBe('build_board_decision_mathematics_engine')
    expect(json.boardDecisionPanel.engine.nextRecommendedMoveId).toBe('product_factory_composer')
    expect(json.boardDecisionPanel.engine.hardGatesBypassed).toBe(0)
    expect(json.boardDecisionPanel.engine.marketLaunchActionDisabled).toBe(true)
