 ...
 *
 * Provides real-time news search using Brave Search API.
 *
 ...
 * @query freshness - Time filter: pd (day), pw (week), pm (month)
 *
...
import { newsSearch, type WebSearchOptions } from "@/lib/integrations/brave-search";
...
export const GET = withErrorBoundary(async (req: NextRequest) => {
  ...
  const options: WebSearchOptions = {
    count,
    freshness: freshness || "pw", // Default to past week for news
  };

  // Execute search
  const results = await newsSearch(query, options);
