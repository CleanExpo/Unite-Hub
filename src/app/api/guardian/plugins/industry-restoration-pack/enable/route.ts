/**
 * Guardian Plugin Marketplace API: Enable Industry Restoration Pack
 *
 * Endpoint: POST /api/guardian/plugins/industry-restoration-pack/enable
 *
 * Enables the Restoration Operations plugin for a workspace.
 * Enforces tier/feature/governance constraints via plugin registry.
 */
...
const PLUGIN_KEY = 'industry_restoration_pack';
...
throw new Error('workspaceId required');
  ...
  // Get workspace details (tier, features, governance)
  ...
  const governance = workspace.governance_settings || {};
  if (plugin.governance.requiresExternalSharing && !governance.allowExternal) {
    return errorResponse(
      'Plugin requires external sharing to be enabled. Update workspace governance settings.',
      403
    );
  }
