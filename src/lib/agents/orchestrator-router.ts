...
const INTENT_PATTERNS: Record<AgentIntent, RegExp[]> = {
  ...
  import_client_emails: [
    /\b(import|sync|fetch|pull)\b.*(emails?|mails?|inbox)/i,
    /\b(emails?|mails?|inbox).*(import|sync|fetch|pull)/i,
    /\b(client|contact)s?\b.*(emails?|communications?)/i,
    /\b(inbox|threads?|messages?).*(sync|import)/i,
    /\b(ingest|ingestion).*(emails?|mails?)/i,
    /\b(sync|import)\b.*(inbox|emails?|mails?)/i,
    /\b(pull|fetch)\b.*(messages?|communications?|inbox)/i,
    /\b(start)\b.*(emails?|mails?).*(ingestion|import|sync)/i,
    /\b(emails?|mails?)\b.*(into|to)\b.*(crm|system|platform)/i,
  ],
  summarise_client_ideas: [
    /\b(summarize|summarise|summary)\b.*(ideas?|actions?|tasks?|clients?)/i,
    /\b(email).?(ideas?|action.?items?|follow.?ups?)/i,
    /\b(extract|pending)\b.*(ideas?|actions?|tasks?|insights?|follow)/i,
    /\b(client)\b.*(intelligence|insights?)/i,
    /\b(communication)\b.*(summary|insights?)/i,
    /\b(email)\b.*(analysis|insights?|report|intelligence)/i,
    /\b(client)\b.*(ideas?|summary|insights?|communications?)/i,
    /\b(pending)\b.*(follow)/i,
    /\b(action).?(items?)/i,
    /\b(follow).?(ups?)/i,
  ],
  manage_social_inbox: [
