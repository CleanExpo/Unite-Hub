 ...
 * Handles caching for alert queries and analytics with fault tolerance
 *
 * Features:
 * - Circuit breaker pattern prevents cascading failures
 * - Automatic fallback to no-cache on Redis failures
 * - Metrics tracking for monitoring
 * - Graceful degradation under high load
 */

import Redis from 'ioredis';
import { CircuitBreaker, createRedisCircuitBreaker, CircuitBreakerError } from './circuit-breaker';
...
class CacheManager {
  ...
  async get<T>(key: string, options?: CacheOptions): Promise<T | null> {
    try {
      ...
      // Execute with circuit breaker, fallback to null (cache miss)
      const value = await this.circuitBreaker.executeWithFallback(
        async () => await this.redis.get(prefixedKey),
        () => null
      );
      ...
      if (error instanceof CircuitBreakerError) {
        ...
        console.warn('[Redis] Circuit breaker open, returning cache miss');
        return null;
      }
  ...
  async set<T>(key: string, value: T, options?: CacheOptions): Promise<void> {
    try {
      ...
      await this.circuitBreaker.executeWithFallback(
        async () => await this.redis.setex(prefixedKey, ttl, JSON.stringify(value)),
        () => undefined // Fallback: silently fail (degraded mode)
      );
      ...
      if (error instanceof CircuitBreakerError) {
        this.metrics.circuitBreakerTrips++;
        console.warn('[Redis] Circuit breaker open, skipping cache set');
        return;
      }
  ...
  /**
   * Get cache health metrics including circuit breaker state
   */
  getMetrics() {
    ...
    return {
      ...
      circuit_breaker: {
        ...
        successes: circuitBreakerStats.successes,
        trips: this.metrics.circuitBreakerTrips,
        is_available: this.circuitBreaker.isAvailable(),
      },
    };
  }
  ...
  /**
   * Check if cache is degraded (circuit breaker open)
   */
  isDegraded(): boolean {
  ...
  async getStatus(): Promise<string> {
    ...
    try {
      const pong = await this.circuitBreaker.execute(async () => await this.redis.ping());
