...
export default function ProductionPerformanceMonitor() {
  ...
  const [lastUpdate, setLastUpdate] = useState<Date> 'use client';
...
.Value -replace "'", "'" <2.5s)`,
      ...
      setAlerts(prevAlerts => 'use client';
...
.Value -replace "'", "'" <= good) return 'text-green-600';
    if (value <= warning) return 'text-yellow-600';
    return 'text-red-600';
  };
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <Button
            ...
            size="sm"
          >
            {isMonitoring ? <PauseCircle className="h-4 w-4 mr-2" /> : <PlayCircle className="h-4 w-4 mr-2" /> 'use client';
...
.Value -replace "'", "'" </Button>
      ...
      {/* Footer */}
      <div className="text-center text-sm text-muted-foreground"> 'use client';
