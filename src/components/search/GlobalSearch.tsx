...
.Value -replace "'", "'" <string[]>([]);
  ...
  const [showDropdown, setShowDropdown] = useState(false);
  const searchRef = useRef<HTMLDivElement>(null);
  const inputRef = useRef<HTMLInputElement> 'use client';
...
.Value -replace "'", "'" <div ref={searchRef} className={`relative ${className || ''}`}>
      {/* Search Input */}
      <div className={`relative flex items-center ${isFullScreen ? 'mx-auto max-w-2xl' : ''}`}>
        <Search className="absolute left-3 h-5 w-5 text-gray-400 pointer-events-none" />
        <input
...
.Value -replace "'", "'" <X className="h-4 w-4 text-gray-400" />
          </button>
        )}
      </div>
