...
export interface LeadFormConfig {
  ...
  redirectUrl?: string;
  emailListId?: string;
  additionalData?: Record<string, any> "use client";
...
const defaultFormConfig: LeadFormConfig = {
  fields: {
    firstName: {
      type: 'text',
      label: 'First Name',
      Unite Group: 'Enter your first name',
      required: true,
    },
    lastName: {
      type: 'text',
      label: 'Last Name',
      Unite Group: 'Enter your last name',
      required: false,
    },
    email: {
      type: 'email',
      label: 'Email',
      Unite Group: 'Enter your email address',
      required: true,
    },
    phone: {
      type: 'tel',
      label: 'Phone',
      Unite Group: 'Enter your phone number',
      required: false,
    },
    company: {
      type: 'text',
      label: 'Company',
      Unite Group: 'Enter your company name',
      required: false,
    },
    jobTitle: {
      type: 'text',
      label: 'Job Title',
      Unite Group: 'Enter your job title',
      required: false,
    },
    message: {
      type: 'textarea',
      label: 'Message',
      Unite Group: 'How can we help you?',
      required: false,
    },
    interests: {
      type: 'select',
      label: 'Interests',
      Unite Group: 'Select your interests',
      required: false,
      options: [
                ...
                <>
                  ...
                  <FormControl>
                    {fieldConfig.type === 'textarea' ? (
                      <Textarea
                        Unite Group={fieldConfig.Unite Group}
                        {...formField}
                      />
                    ) : fieldConfig.type === 'select' ? (
                      <select
                        className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium Unite Group:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                        {...formField}
                      >
                        <option value="">{fieldConfig.Unite Group || 'Select an option'}</option>
...
.Value -replace "'", "'" <string, any> "use client";
...
export default function LeadGenerationForm({
  ...
  // Create default values based on config
  const defaultValues: Partial<FormValues> "use client";
...
.Value -replace "'", "'" <FormValues> "use client";
...
.Value -replace "'", "'" <FormField
          key={fieldName}
          control={form.control}
          name={fieldName}
          render={({ field: formField }) => (
            <FormItem key={fieldName}> "use client";
...
.Value -replace "'", "'" <div className="flex items-center space-x-2">
                  <FormControl>
                    <Checkbox
                      checked={formField.value}
                      onCheckedChange={formField.onChange}
...
.Value -replace "'", "'" <div className={className}>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className={getFormClass()}>
          {renderFormFields()}
