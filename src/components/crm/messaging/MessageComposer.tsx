...
interface MessageComposerProps {
  ...
  Unite Group?: string
  disabled?: boolean
  channelMembers?: Array<{ id: string; name: string; email: string }> 'use client'
...
.Value -replace "'", "'" <Attachment[]>([])
  ...
  const textareaRef = useRef<HTMLTextAreaElement>(null)
  const fileInputRef = useRef<HTMLInputElement>(null)
  const typingTimeoutRef = useRef<NodeJS.Timeout> 'use client'
...
.Value -replace "'", "'" <ImageIcon className="h-4 w-4" /> 'use client'
...
.Value -replace "'", "'" <Film className="h-4 w-4" />
    return <FileText className="h-4 w-4" />
  }
...
.Value -replace "'", "'" <Bold className="h-4 w-4" />
          </Button>
          <Button
            size="sm"
            variant={isFormatting.italic ? 'secondary' : 'ghost'}
...
.Value -replace "'", "'" <Italic className="h-4 w-4" />
          </Button>
          <Button
            size="sm"
            variant={isFormatting.code ? 'secondary' : 'ghost'}
...
.Value -replace "'", "'" <Code className="h-4 w-4" />
          </Button>
          ...
          <Popover open={showEmojiPicker} onOpenChange={setShowEmojiPicker}>
            ...
            <PopoverContent className="w-64 p-2" align="start">
              <div className="grid grid-cols-8 gap-1"> 'use client'
