...
export default function ClientAnalytics({ clientId }: ClientAnalyticsProps) {
  ...
  const [segments, setSegments] = useState<ClientSegment[]> "use client";
...
.Value -replace "'", "'" < 30 days',
        count: clientsData.filter(c => "use client";
              ...
              {clients
                ...
                .map((client) => (
                  <div key={client.id} className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      <h4 className="font-medium">{client.name}</h4>
                      <p className="text-sm text-muted-foreground"> "use client";
