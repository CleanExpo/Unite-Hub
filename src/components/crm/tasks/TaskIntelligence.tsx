...
export default function TaskIntelligence({ projectId }: TaskIntelligenceProps) {
  ...
  const [metrics, setMetrics] = useState<TaskMetrics> "use client";
...
.Value -replace "'", "'" < now;
    ...
    // Calculate status breakdown
    const statusBreakdown: Record<string, number> "use client";
...
.Value -replace "'", "'" < now;
  ...
  // Get recent tasks
  const getRecentTasks = () => "use client";
...
.Value -replace "'", "'" <Badge className={variants[priority as keyof typeof variants] || variants.medium}>
      {priority.toUpperCase()}
    </Badge> "use client";
...
.Value -replace "'", "'" <Badge className={variants[status as keyof typeof variants] || 'bg-gray-100 text-gray-800'}> "use client";
                ...
                <div className="space-y-4">
                  ...
                  {Object.entries(metrics.statusBreakdown).map(([status, count]) => (
                    <div key={status} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex items-center space-x-3">
                        <div>
                          <h5 className="font-medium capitalize"> "use client";
            ...
            <CardContent>
              <div className="space-y-4">
                {Object.entries(metrics.statusBreakdown).map(([status, count]) => (
                  <div key={status} className="space-y-2">
                    <div className="flex justify-between">
                      <span className="capitalize font-medium"> "use client";
