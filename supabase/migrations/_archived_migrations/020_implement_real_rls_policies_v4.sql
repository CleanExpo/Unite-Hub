-- =====================================================
-- MIGRATION 020 V4: Implement REAL Row Level Security Policies
-- =====================================================
-- Date: 2025-01-17
...
BEGIN
  ...
  IF workspace_org_id_type = 'uuid' THEN
    -- workspaces.org_id is UUID - direct comparison
    EXECUTE '
  ...
  ELSE
    -- workspaces.org_id is TEXT - cast to text
    EXECUTE '
    CREATE POLICY "Users can view workspaces in their orgs"
      ON workspaces FOR SELECT
      USING (
    ...
    RAISE NOTICE 'Created workspace policies (TEXT mode with casting)';
  END IF;
END $$;
...
COMMENT ON FUNCTION user_has_role_in_org(UUID, TEXT) IS 'Role hierarchy check for organizations';
