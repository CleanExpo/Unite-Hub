...
drop index if exists idx_schedule_campaign;
...
drop index if exists idx_schedule_send_at;
...
drop index if exists idx_analytics_campaign;
...
-- Updated_at triggers
drop trigger if exists update_schedule_timestamp on synthex_campaign_schedule;
create trigger update_schedule_timestamp
  before update on synthex_campaign_schedule
  for each row execute function update_updated_at_column();

drop trigger if exists update_analytics_timestamp on synthex_campaign_analytics;
...
-- Schedule policies
drop policy if exists "schedule_select" on synthex_campaign_schedule;
create policy "schedule_select" on synthex_campaign_schedule
  for select using (auth.uid() = user_id);

drop policy if exists "schedule_insert" on synthex_campaign_schedule;
create policy "schedule_insert" on synthex_campaign_schedule
  for insert with check (auth.uid() = user_id);

drop policy if exists "schedule_update" on synthex_campaign_schedule;
create policy "schedule_update" on synthex_campaign_schedule
  for update using (auth.uid() = user_id);

drop policy if exists "schedule_delete" on synthex_campaign_schedule;
create policy "schedule_delete" on synthex_campaign_schedule
  for delete using (auth.uid() = user_id);

-- Analytics policies (tenant-based for team access)
drop policy if exists "analytics_select" on synthex_campaign_analytics;
...
drop policy if exists "analytics_insert" on synthex_campaign_analytics;
...
drop policy if exists "analytics_update" on synthex_campaign_analytics;
...
-- Insights policies
drop policy if exists "insights_select" on synthex_insights;
create policy "insights_select" on synthex_insights
  for select using (auth.uid() = user_id);

drop policy if exists "insights_insert" on synthex_insights;
create policy "insights_insert" on synthex_insights
  for insert with check (auth.uid() = user_id);

drop policy if exists "insights_update" on synthex_insights;
create policy "insights_update" on synthex_insights
  for update using (auth.uid() = user_id);

drop policy if exists "insights_delete" on synthex_insights;
