-- =====================================================
-- MIGRATION 020 CORE TABLES: Implement RLS for 9 core tables
-- =====================================================
-- Skips generatedContent and auditLogs (don't exist yet)
...
CREATE FUNCTION user_has_role_in_org_simple(p_org_id TEXT, p_required_role TEXT)
RETURNS BOOLEAN AS $$
DECLARE
  v_user_role TEXT;
...
BEGIN
  ...
  RAISE NOTICE 'Tables with RLS: % / 9', rls_enabled_count;
  IF rls_enabled_count >= 9 THEN
    RAISE NOTICE '✅ SUCCESS: RLS restored on 9 core tables';
