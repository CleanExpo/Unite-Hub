-- ============================================================================
-- Migration 555 (CORRECTED v2): Multi-Tenant RLS Policies
-- ============================================================================
-- Date: December 9, 2025
-- Status: Fixes v1 which failed on non-existent whatsapp_messages table
--
-- Changes from v1:
--   - Wraps each table's policies in existence check
--   - Only creates policies if table exists
--   - Safe to run even if some tables missing
-- ============================================================================
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'whatsapp_messages') THEN
    ...
    RAISE NOTICE 'RLS policies created for whatsapp_messages';
  ELSE
    RAISE NOTICE 'Skipping whatsapp_messages - table does not exist';
  END IF;
END $$;
