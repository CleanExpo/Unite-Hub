...
ALTER TABLE user_profiles
  ...
  ADD COLUMN IF NOT EXISTS notification_preferences JSONB DEFAULT '{"email_notifications":true,"marketing_emails":true,"product_updates":true,"weekly_digest":false}'::jsonb;-- Add unique constraint on username (separate from column definition)
DO $$
BEGIN
...
BEGIN
  ...
  END IF;
END $$;-- Create index for username lookups
CREATE INDEX IF NOT EXISTS idx_user_profiles_username ON user_profiles(username) WHERE username IS NOT NULL;-- Add comment
COMMENT ON COLUMN user_profiles.username IS 'Unique username for the user (3-30 chars, alphanumeric, dash, underscore)';COMMENT ON COLUMN user_profiles.business_name IS 'Business or company name';COMMENT ON COLUMN user_profiles.phone IS 'Phone number in international format';COMMENT ON COLUMN user_profiles.bio IS 'User biography (max 500 characters)';COMMENT ON COLUMN user_profiles.website IS 'User or business website URL';COMMENT ON COLUMN user_profiles.timezone IS 'User timezone (IANA timezone identifier)';COMMENT ON COLUMN user_profiles.notification_preferences IS 'User notification preferences (JSON)';