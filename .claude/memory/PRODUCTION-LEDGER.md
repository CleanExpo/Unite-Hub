...
- **🔴→✅ RETRACTED prod-config finding (was false — corrected 2026-05-30):** An earlier note here claimed prod `NEXT_PUBLIC_SUPABASE_URL` was empty and that production had no Supabase connection. **That was wrong — a `vercel env pull` CLI artifact, not the real state.** Proof: (1) `vercel env ls production` lists all 9 required vars as present + **Encrypted** (created ~10h prior), including `NEXT_PUBLIC_SUPABASE_URL`; (2) the live site returns a clean `HTTP 307 → /auth/login?redirectTo=%2F` (not a 500 — auth middleware is reaching Supabase); (3) a fresh `vercel env pull` rendered **all 30 variables empty** — including known-set CRITICAL vars `ANTHROPIC_API_KEY` and `VAULT_ENCRYPTION_KEY` — confirming `pull`-empty is a rendering bug, never evidence of unset vars. **Authoritative source is `vercel env ls`, not `pull`.** Prod env config is intact; the "/shipit SHIPPED, prod live" claim stands.

- **✅ RESOLVED 2026-05-30 — prod DB identity: prod == sandbox (`lksfwktwtmyznckodsau`).** The last prod-DB caveat (do the #1/#2 drift migrations exist in prod?) is closed. Proof: (1) the live prod client bundle at `unite-hub-self.vercel.app` embeds `lksfwktwtmyznckodsau.supabase.co` (`NEXT_PUBLIC_SUPABASE_URL` is public, baked into client JS — found by grepping all 16 JS chunks for `[a-z0-9-]*\.supabase\.(co|in)`); (2) `vercel env pull` again rendered it empty — re-confirming pull is unreliable, the bundle is authoritative; (3) direct `execute_sql` on `lksfwktwtmyznckodsau` shows `hub_satellites` table LIVE + `approval_queue.founder_id` column LIVE → **the 30/05 drift-repair migrations ARE live in prod. No replay needed.** The contrarian "prod ≠ sandbox, 30 missing migrations" risk branch is disproven. **New (non-blocking) observation:** that Supabase project reports **1,710 public tables** — far beyond this CRM's ~30; the project is shared/contaminated with other apps' tables. Flag for later hygiene; does not affect founder-scoped RLS correctness.
