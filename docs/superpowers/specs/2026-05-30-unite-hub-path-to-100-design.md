...
- **0.4** ✅ DONE 2026-05-30 — audited all **46 service-role routes** for explicit founder_id (service role bypasses RLS → app filter is the only line) on branch `fix/video-generate-founder-scope-phase0-4`. Result: **0 exploitable IDOR**, but **1 defence-in-depth fix** — `video/generate/route.ts` had two `video_assets` `.update(...).eq('id', videoAsset.id)` chains (generating + failed paths) missing `founder_id`. Fixed via TDD (2 red→green tests asserting the filter on both paths). Audit breakdown: 34 SAFE (explicit `.eq('founder_id', …)` or self-insert with `founder_id` in payload), 10 CRON (CRON_SECRET-gated, FOUNDER_USER_ID-scoped, no HTTP IDOR surface), 1 fixed (video/generate), 1 verified-by-hand (`xero/callback` — `credentials_vault.upsert({founder_id: user.id}, {onConflict:'founder_id,service,label'})`, self-write of own row, SAFE). 576/576 tests; type-check + lint clean.
  - *Audit-method note:* the first automated triage pass reported "0 violations" but (a) silently dropped `xero/callback` from its table and (b) applied an IDOR-only lens that missed video/generate's defence-in-depth gap. Both were caught and corrected by hand against `rules/database/supabase.md`'s "Never bypass founder_id, even in service-role contexts."
- **Acceptance:** prod ref recorded ✓; drift migrations confirmed applied-as-migrations ✓; route scan returns 0 VIOLATION routes (0.3) ✓; 46 service-role routes audited, 0 IDOR + defence-in-depth gap closed (0.4) ✓; verify loop green ✓. **Phase 0 COMPLETE.**
