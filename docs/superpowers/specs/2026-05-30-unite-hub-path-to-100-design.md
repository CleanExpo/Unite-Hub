...
- **0.4** ✅ DONE 2026-05-30 — audited all **46 service-role routes** for explicit founder_id (service role bypasses RLS → app filter is the only line). Result: **0 exploitable IDOR, 2 defence-in-depth fixes** (both: founder-owned id by construction, not URL-supplied → not exploitable, but the `.update()` chains lacked the explicit `founder_id` filter the invariant mandates):
  ...
  - *Scope of the structural scan:* covers the `.eq('id', …)` single-row signature (where G4 + these 2 gaps lived). A broader sweep (every founder-table `.from()` without founder_id, incl. `business_key`/`campaign_id`-keyed) rests on the subagent table for the non-id-keyed routes — flagged as residual audit surface, not yet independently verified.
- **Acceptance:** prod ref recorded ✓; drift migrations confirmed applied-as-migrations ✓; route scan returns 0 VIOLATION routes (0.3) ✓; 46 service-role routes audited, 0 IDOR + both defence-in-depth gaps closed (0.4) ✓; verify loop green ✓. **✅ PHASE 0 CLOSED 2026-05-30 — PR #67 merged (`cf794fe4`).**
